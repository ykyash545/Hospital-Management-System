...
# Function to create a table if it doesn't exist for tickets
...
def add_ticket(ticket_number, description, status):
    conn = create_connection()
    with conn:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO tickets (ticket_number, description, status) VALUES (%s, %s, %s)", (ticket_number, description, status))
...
def view_tickets():
    ...
    if not rows:
        ...
        st.write('## Tickets')
        for row in rows:
            st.write(f"**ID:** {row[0]}, **Ticket Number:** {row[1]}, **Description:** {row[2]}, **Status:** {row[3]}")
...
def view_appointments():
    ...
    if not rows:
        ...
        for row in rows:
            st.write(f"**ID:** {row[0]}, **Patient ID:** {row[1]}, **Doctor ID:** {row[2]}, **Date:** {row[3]}, **Time:** {row[4]}")

# Function for appointments management
def appointments():
    st.subheader("Appointments Management")

    # Option to delete an appointment
    st.subheader("Delete Appointment")
    appointment_id_to_delete = st.number_input('Enter ID of Appointment to Delete', min_value=1, step=1)
    if st.button('Delete Appointment'):
        delete_appointment(appointment_id_to_delete)
...
def main():
    ...
    # Navigation
    menu = ['Create record', 'User Management', 'Patient Management', 'Doctor Management', 'Generate Invoice', 'Inventory', 'Ticket Management', 'Appointments']
    ...
    # Add the following conditions for ticket management
    elif choice == 'Ticket Management':
        st.subheader("Ticket Management")
        menu = ['Add Ticket', 'View Tickets', 'Delete Ticket']
        choice = st.sidebar.selectbox('Menu', menu)

        if choice == 'Add Ticket':
            ...
            if st.button('Add Ticket') and ticket_number and description:
                add_ticket(ticket_number, description, status)
            ...
            ticket_id = st.number_input('Enter ID of Ticket to Delete', min_value=1, step=1)
